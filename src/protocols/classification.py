"""
Classification protocols for query intent detection.

This module defines the Protocol-based interfaces for classification strategies,
following the Strategy pattern for pluggable classification implementations.

Classification Chain (from fastest to slowest):
...
Example:
    >>> from src.protocols.classification import ClassificationStrategy, ClassificationResult
    >>>
    >>> class MyClassifier:
    ...     async def classify(self, query: str, user_id: str, context: dict | None = None) -> ClassificationResult:
    ...         return ClassificationResult(intent="rag", confidence=0.9)
"""
...
class ClassificationStrategy(Protocol):
    ...
    This protocol defines the interface for all classification strategies,
    enabling the Strategy pattern for pluggable classification implementations.
...
class ClassificationCache(Protocol):
    """
    Protocol for classification result caching.
    ...
    Example:
        ...
        ...
        ...     def set(self, key: str, value: ClassificationResult, ttl: int) -> None:
