"""
ABC definitions for dependency injection container.

This module provides ABC (Abstract Base Class) versions of container protocols,
enforcing stricter interface compliance with @abstractmethod decorators.
...
Example:
    ...
    >>>
    >>> class RequestScopeManager(ScopeManagerABC):
    ...     def __init__(self):
...
from typing import TypeVar, Type, TYPE_CHECKING, Any, Callable, Awaitable
...
class DependencyContainerABC(ABC):
    """
    ABC for dependency injection container.

    Enforces implementation of all DI container methods using @abstractmethod.
    Matches DependencyContainer protocol exactly for equivalence.
    ...
    async def register_factory(
        ...
        Example:
            >>> def create_classifier(container: DependencyContainerABC) -> ClassificationStrategy:
...
class ServiceRegistryABC(ABC):
    """
    ABC for service registration and container initialization.

    Enforces implementation of service registry methods using @abstractmethod.
    Matches ServiceRegistry protocol exactly for equivalence.

    Example:
        >>> class MyServiceRegistry(ServiceRegistryABC):
        ...     @staticmethod
        ...     async def initialize_container() -> DependencyContainerABC:
        ...         container = ServiceContainer()
        ...         await container.register_singleton(ClassificationStrategy, CompositeClassifier)
        ...         await container.register_singleton(QueryHandler, RAGHandler)
        ...         return container
        ...
        ...     async def warm_up(self, container: DependencyContainerABC) -> None:
    ...
    async def initialize_container() -> "DependencyContainerABC":
        ...
        Example:
            >>> container = await ServiceRegistryABC.initialize_container()
            >>> classifier = await container.get(ClassificationStrategy)
        """
        ...
...
class ScopeManagerABC(ABC):
    """
    ABC for managing scoped services.

    Enforces strict implementation with @abstractmethod decorators.
    Matches ScopeManager protocol exactly for equivalence.
