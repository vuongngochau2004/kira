"""RAGAS evaluation-related schemas for RAG system assessment."""

from pydantic import BaseModel, Field, field_validator
from datetime import datetime
from enum import Enum
from typing import Any  # Keep Any for dynamic dict values
...
class EvaluationRequest(BaseModel):
    ...
    @classmethod
    def validate_contexts_not_empty(cls, v: list[str]) -> list[str]:
        """Validate that all context strings are non-empty."""
        for i, context in enumerate(v):
            if not context or not context.strip():
                raise ValueError(f"Context at index {i} is empty. All contexts must be non-empty strings.")
        return v
