...
export function SourcePanel({ isOpen, onToggle }: SourcePanelProps) {
  ...
  const activeChunkInfo = useMemo(() => {
    ...
    if (currentId) {
      // Try finding in full document chunks
  ...
  useEffect(() => {
    if (previewTab === 'text' && activeChunkInfo?.id) {
      setTimeout(() => {
        const el = document.getElementById(`preview-chunk-${activeChunkInfo.id}`)
        if (el) {
                        ...
                        <div className="mt-2 text-xs text-muted-foreground relative">
                          {isCardExpanded ? (
                            <div className="flex flex-col gap-2">
                              {group.document_id && loadingDocs[group.document_id] ? (
                                <div className="flex items-center gap-2 py-4 justify-center text-muted-foreground">
                                  <Loader2 className="w-4 h-4 animate-spin text-primary" />
                                  <span>Đang tải tài liệu...</span>
                                </div>
                              ) : group.document_id && docChunks[group.document_id] ? (
                                <div className="border border-border/80 bg-muted/10 rounded-lg p-3 max-h-60 overflow-y-auto custom-scrollbar text-xs leading-relaxed text-foreground select-text space-y-4">
                                  {Object.entries(
                                  ...
                                  .filter(([pageNum, pageChunks]) => {
                                    return pageChunks.some(chunk => 
                                      group.chunks.some(c => {
                                        const cleanedS = c.snippet.trim().toLowerCase()
                                      ...
                                      <div className="text-justify leading-relaxed">
                                        {pageChunks.map((chunk) => {
                                          ...
                                          const isExactCurrentSource = (() => {
                                            const selectedId = localActiveChunkId || store.activeSourceId
                                            if (selectedId) {
                                              if (selectedId === chunk.id) return true
                                              const matchedS = sources.find(s => s.id === selectedId)
                                              if (matchedS && (matchedS.document_id === group.document_id || matchedS.title === group.title)) {
                                                const cleanedS = matchedS.snippet.trim().toLowerCase()
                                                const cleanedC = chunk.content.trim().toLowerCase()
                                                return cleanedC.includes(cleanedS) || cleanedS.includes(cleanedC)
                                              }
                                            }
                                          ...
                                          return (
                                            <span
                                              ...
                                              className={cn(
                                                "transition-colors cursor-pointer px-0.5 rounded",
                                              ...
                                              title={matchesAnySource ? "Đoạn trích dẫn" : "Văn bản gốc"}
                                            >
                                              {chunk.content}{" "}
                                            </span>
                                          )
                                ...
                                <div className="border border-border/80 bg-muted/10 rounded-lg p-2.5 max-h-56 overflow-y-auto custom-scrollbar text-xs space-y-2">
                                  {group.chunks.map((c, chunkIdx) => {
                                    ...
                                    return (
                                      <p 
                                        ...
                                        className={cn(
                                          "leading-relaxed cursor-pointer pl-2 py-1 border-l-2 rounded-r-md font-bold",
                                          isExactCurrentSource
                                            ? "text-foreground bg-primary/25 border-primary shadow-sm"
                                            : "text-foreground bg-primary/10 border-primary/50 hover:bg-primary/15"
                                        )}
                                      >
                                        {c.snippet}
                                      </p>
                            ...
                            {Object.entries(chunksByPage).map(([pageNum, pageChunks]) => (
                              <div key={pageNum} className="space-y-2">
                                ...
                                <p className="indent-6 text-justify leading-relaxed">
                                  {pageChunks.map((chunk) => {
                                    const isSelected = activeChunkInfo?.id === chunk.id
                                    const matchesAnySource = activeGroup.chunks.some(c => {
                                      const cleanedS = c.snippet.trim().toLowerCase()
                                      const cleanedC = chunk.content.trim().toLowerCase()
                                      return cleanedC.includes(cleanedS) || cleanedS.includes(cleanedC)
                                    })
                                    ...
                                    return (
                                      <span
                                        ...
                                        className={cn(
                                          ...
                                          isSelected
                                            ? "bg-primary/30 ring-1 ring-primary/40 font-bold text-foreground"
                                            : matchesAnySource
                                              ? "bg-primary/15 hover:bg-primary/25 font-semibold text-foreground/90"
